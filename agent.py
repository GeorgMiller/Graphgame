...
class Agent():
    ...
    def __init__(self, 
        ...
        self.hypernetwork = hypernetwork.Hypernetwork(self.weights_init)
    ...
    def play_game(self, thread):
        ...
        while not done:
            ...
            if state[0][4] == 1:
                if state[0][12]==1 or state[0][13]==1 or state[0][14]==1 or state[0][15]==1:
                    left = 1
            if state[0][12] == 1:
                if  state[0][4] ==1 or state[0][5]==1 or state[0][6]==1 or state[0][7]==1:
                    right = 1
            ...
            if done:
                ...
                self.right_average.append(sum(self.right[-50:]) / len(self.right[-50:]))
                self.left_average.append(sum(self.left[-50:]) / len(self.left[-50:]))
    ...
    def create_actor(self):
        ...
        model = tf.keras.Sequential([
            Dense(64, activation='relu', kernel_initializer=kernel_init, bias_initializer=bias_init, input_shape=(16,)),
